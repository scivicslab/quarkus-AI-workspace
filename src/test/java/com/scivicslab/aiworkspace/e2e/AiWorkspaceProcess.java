...
/**
 * Starts the quarkus-AI-workspace uber-JAR as a child process for integration tests.
 *
...
public class AiWorkspaceProcess {
    ...
    private AiWorkspaceProcess(Process process, int port, File logFile) {
    ...
    /**
     * Start quarkus-AI-workspace with an explicit jarsDir that overrides TEST_JARS_DIR.
     * Useful when the test prepares a temp dir with correctly-named JARs.
     */
    public static AiWorkspaceProcess start(Path configYaml, int port,
                                             Map<String, String> extraEnv) throws Exception {
        String jarPath = resolveDefaultJar();
        File   jarFile = new File(jarPath);
        if (!jarFile.exists()) {
            throw new IllegalStateException(
                "quarkus-AI-workspace JAR not found: " + jarFile.getAbsolutePath()
        ...
        System.out.println("  [quarkus-ai-workspace:" + port + "] log → " + logFile.getAbsolutePath());
        AiWorkspaceProcess spp = new AiWorkspaceProcess(proc, port, logFile);
        spp.waitForReady(20_000);
        return spp;
    }

    public static AiWorkspaceProcess start(Path configYaml, int port) throws Exception {
    ...
    private void waitForReady(long timeoutMs) throws Exception {
        ...
        while (System.currentTimeMillis() < deadline) {
            if (!process.isAlive()) {
                throw new IllegalStateException(
                    "quarkus-AI-workspace died during startup. Log: " + logFile.getAbsolutePath());
            }
            try {
        ...
        throw new IllegalStateException(
            "quarkus-AI-workspace did not respond within " + timeoutMs + "ms");
    }
