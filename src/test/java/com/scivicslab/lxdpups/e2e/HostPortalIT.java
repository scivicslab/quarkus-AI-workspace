...
public class HostPortalIT {
    ...
    void kafkaStatusIsInactive() {
        var api = playwright.request().newContext();
        var body = api.get(baseUrl + "/api/status").text();
        // Find the kafka entry and verify it's INACTIVE (Kafka not installed yet)
        ...
        assertTrue(kafkaSection.contains("INACTIVE"), "kafka should be INACTIVE (not yet installed)");
        api.dispose();
    }
    ...
    void kafkaStartButtonVisible() {
        page.navigate(baseUrl);
        // There should be a Start button for kafka
        var startButtons = page.locator("button:has-text('Start'), a:has-text('Start')");
        assertTrue(startButtons.count() > 0, "At least one Start button should be visible");
    }
