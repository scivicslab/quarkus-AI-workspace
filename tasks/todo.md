...
- ユニットテスト 30 件（既存 9 + 新規 21）、全件緑。全体では 96 件。
- E2E `ConversationTurnNavigationE2E` 43 項目、全件緑。テスト自身が固定データの会話 DB を作り、
...
chat-ui3 の Sessions が読みやすいのは、単位が「方向つきメッセージ」だから。1 エントリが 2 通に
分かれる。llm は `loop → LLM`(REQUEST) と `LLM → loop`(RESPONSE/REASONING/TOOL_CALLS)、
tool は `loop → tool`(TOOL/INPUT) と `tool → loop`(OBSERVATION)。turn の頭に `user → loop`。
「call」はこれを丸めた単位で、方向が消えていた。分割は Java 側で行い、画面はパースしない。

右ペインは `flex: 0 0 45%` の一覧と残り 55% の本文に固定。伸縮に任せないので一方が他方を潰せない。

実データから 2 点学んだ。(1) `user → loop` の中身がシステムプロンプトになる。このプログラムは
システムプロンプト・ツール説明・質問を 1 つの user メッセージ(7,905 文字)に畳んでおり、質問は末尾。
`user → loop` だけ末尾から要約する。ロールを分けるプログラムでも最後の user メッセージが質問なので
どちらでも質問が出る。(2) マーカーは行頭のものだけを認める。人の発言に `RESPONSE:` と書かれていると
リクエストの途中で答えが始まったと誤認する。chat-ui3 の分割はこの点で誤りうる。

実測: 30 エントリの turn は 91 メッセージ。この状態でも 4 kB のメッセージを開いた状態でも
ページはウィンドウより高くならない。一覧に 9 行が丸ごと見え、先頭行は 0 px も切れない。
一覧が持つのは要約 201 文字、開いた本文は 4,066 文字。

### 一覧が 11 px に潰れる件

`.conv-calls` に `flex: 0 1 auto` を指定していたため、下の本文と場所を取り合って一覧が潰れた。
入れ物 11 px に対し 1 行 27 px。E2E が捕まえられなかったのは、ウィンドウに対してしか測って
いなかったから。高さ 11 px の入れ物もウィンドウの内側には収まる。入れ物を基準にした測定に変えた。
